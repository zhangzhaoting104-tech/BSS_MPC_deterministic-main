...
mutable struct QuasiStaticSolver
    m::Model
    csp_s::VariableRef
    csn_s::VariableRef
    phi_p::VariableRef
    ...
    cf0::VariableRef
    csp_avg1::VariableRef
    csn_avg1::VariableRef
    delta_sei1::VariableRef
    cf1::VariableRef
    power::VariableRef
end

function get_quasi_static_solver()
    ...
    @constraint(m, (csp_avg1 .- csp_avg0) ./ DT_STATE == -15 .* Dp ./ Rpp ./ Rpp .* (csp_avg0 .- csp_s))
    @constraint(m, (csn_avg1 .- csn_avg0) ./ DT_STATE == -15 .* Dn ./ Rpn ./ Rpn .* (csn_avg0 .- csn_s))
    @constraint(m, (delta_sei1 .- delta_sei0) ./ DT_STATE == isei .* M_sei ./ F ./ rho_sei ./ an ./ lnn)
    @constraint(m, (cf1 .- cf0) ./ DT_STATE == isei ./ 3600)

    return QuasiStaticSolver(m, csp_s, csn_s, phi_p, phi_n, it, iint, Up, Un, theta_p, theta_n, csp_avg0, csn_avg0, delta_sei0, cf0, csp_avg1, csn_avg1, delta_sei1, cf1, power)
end


function reset_solver!(solver::QuasiStaticSolver, u0, soc_target)
    ...
    set_start_value(solver.it, 0.0)
    set_start_value(solver.iint, 0.0)
    ...
    fix(solver.cf0, cf0)
    fix(solver.csn_avg1, capacity_remain * soc_target * csnmax)
end


function compute_quasi_static(solver::QuasiStaticSolver, u0, soc_target)
    start_time = time()
    reset_solver!(solver, u0, soc_target)
    optimize!(solver.m)
    status = is_solved_and_feasible(solver.m)
    if !status
        println("quasi-static solver failed, elapsed time:  ", time() - start_time)
        exit()
    end

    power = value(solver.power)
    ...
    isei0 = it0 - iint0
    csp_avg1 = value(solver.csp_avg1)
    csn_avg1 = value(solver.csn_avg1)
    delta_sei1 = value(solver.delta_sei1)
    cf1 = value(solver.cf1)
