...
state_input = Matrix(df[:, [:x1, :x2, :x3, :x4, :u]])
next_state = Matrix(df[:, [:x_next1, :x_next2, :x_next3, :x_next4]])
...
global θ_csp_avg = []
global num_obs_per_iter_csp_avg = 5
...
global θ_csn_avg = []
global num_obs_per_iter_csn_avg = 5
global num_obs_added = 1
...
global θ_delta_sei = []
global num_obs_per_iter_delta_sei = 5
global num_obs_added = 1
...
err = zero(y)
for i in 1:4
    x_test = [collect(row) for row in eachrow(x)]
    y_pred = zero(y_cf)
    for j in 1:total_data_length
        y_pred[j] = pred(krigin_surrogate[i], x_test[j] .* vec(x_stds) .+ vec(x_means))
    end
    err[:, i] = abs.(y_pred .+ state_input[:, i] .- next_state[:, i])
end
