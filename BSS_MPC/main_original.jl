...
optimal_control_solver = get_optimal_control_solver(krigin_surrogate)
...
for hour in 1:total_iters
    ...
    # solve ocp
    CF_MIN = get_minimum_cf(swap_battery_times, u0[:, end])
    bat_charge, eps = compute_ocp_solution(optimal_control_solver, u0, grid_price, swap_battery_times, swap_battery_states, hour, CF_MIN)
...
record("../record/original_low_cf.csv", cf_avg, cf_std, cumulative_profit, num_soc_violate_list)
